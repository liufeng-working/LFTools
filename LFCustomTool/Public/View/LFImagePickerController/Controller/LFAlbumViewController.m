...
- (void)viewDidLoad {
    ...
    if (self.status == PHAuthorizationStatusNotDetermined) {//用户正在授权
        ...
        [self showAlertWithTitle:@"温馨提示" message:@"照片库访问受限" cancelTitle:@"取消" sureTitle:@"去设置" complete:^{//点击设置按钮后的处理逻辑
...
{
    //同步方法
    [[LFImageManager manager] allAlbums:lfImagePickerController.showVideo completion:^(NSArray<LFAlbumModel *> *albumModels) {
