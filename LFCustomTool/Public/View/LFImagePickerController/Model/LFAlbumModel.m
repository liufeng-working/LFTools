...
#pragma mark -
#pragma mark - 构建实例对象
- (instancetype)initWithFetchResult:(PHFetchResult<PHAsset *> *)fetchResult assetCollection:(PHAssetCollection *)assetCollection
{
    self = [super init];
    if (self) {
        _title = [self transformAblumTitle:assetCollection.localizedTitle];
        _count = fetchResult.count;
        _assetMs = [self assetModels:fetchResult];
        _selectAssetMs = [NSArray array];
        _rollAlbum = assetCollection.assetCollectionType == PHAssetCollectionTypeSmartAlbum && assetCollection.assetCollectionSubtype == PHAssetCollectionSubtypeSmartAlbumUserLibrary;
    }
    return self;
}

+ (instancetype)albumModel:(PHFetchResult<PHAsset *> *)fetchResult assetCollection:(PHAssetCollection *)assetCollection
{
    return [[self alloc] initWithFetchResult:fetchResult assetCollection:assetCollection];
}
